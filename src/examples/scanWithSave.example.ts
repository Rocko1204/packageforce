...
import { DuplicateDetectorService, DuplicateDetectionOptions } from '../services/duplicateDetectorService';
...
async function cicdExample() {
  ...
  try {
    ...
    const duplicateResult = await duplicateService.findDuplicates(duplicateOptions);
    ...
    if (duplicateResult.totalDuplications > 0 && duplicateResult.hasCrossPackageDuplicates) {
      console.error(`Found ${duplicateResult.totalDuplications} duplicate code blocks`);
      process.exit(1);
    }
