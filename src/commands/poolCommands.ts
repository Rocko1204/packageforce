...
export async function fetchScratchOrgFromPool(): Promise<void> {
  ...
  try {
    ...
    const enableTracking = await vscode.window.showQuickPick(['No', 'Yes'], {
    ...
    logger.info(
      `Source tracking: ${setSourceTracking ? 'enabled' : 'disabled'}`
    );
      ...
      async progress => {
        try {
          ...
          try {
            ...
            throw new Error(
              `Dev Hub connection failed: ${error instanceof Error ? error.message : 'Unknown error'}`
            );
          }
          ...
          progress.report({
            increment: 30,
            message: 'Searching for available scratch orgs...',
          });
          logger.info(
            `Step 2/3: Searching for available scratch orgs with tag '${tag}'...`
          );
          ...
          vscode.window
            .showInformationMessage(message, 'Open Org', 'View Logs')
            .then(selection => {
              if (selection === 'Open Org') {
                vscode.commands.executeCommand('sfdx.force.org.open', {
                  username,
                });
          ...
          vscode.window
            .showErrorMessage(
              `Failed to fetch scratch org: ${errorMessage}`,
              'View Logs'
            )
            .then(selection => {
...
export async function listScratchOrgsInPool(): Promise<void> {
  try {
        ...
        try {
          ...
          if (scratchOrgs.length === 0) {
            ...
            vscode.window.showInformationMessage(
              `No scratch orgs found with tag '${tag}'`
            );
          ...
          scratchOrgs.forEach((org, index) => {
            logger.info(
              `[${index + 1}] ${poolService.formatScratchOrgInfo(org)}`
            );
          ...
          vscode.window
            .showErrorMessage(
              `Failed to list scratch orgs: ${errorMessage}`,
              'View Logs'
            )
            .then(selection => {
              if (selection === 'View Logs') {
    ...
    vscode.window
      .showErrorMessage('An unexpected error occurred', 'View Logs')
      .then(selection => {
        if (selection === 'View Logs') {
          logger.show();
        }
