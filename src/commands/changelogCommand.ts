...
import { ChangelogOptions, PackageChangeInfo, PluginSettings } from '../utils/changelogTypes';
...
export async function executeChangelogCommand() {
  try {
    ...
    try {
      projectJson = await SfProjectJson.create({ rootFolder: workspaceFolder.uri.fsPath });
      projectContents = projectJson.getContents();
      settings = projectContents.plugins?.['eon-sfdx'] as PluginSettings | undefined;
      
      logger.info(`Loaded sfdx-project.json successfully`);
      logger.info(`Found ${projectContents.packageDirectories?.length || 0} package directories`);
      
      if (!projectContents.packageDirectories || projectContents.packageDirectories.length === 0) {
        vscode.window.showErrorMessage('No package directories found in sfdx-project.json');
        return;
      }
    } catch (error) {
      logger.error('Failed to load sfdx-project.json:', error);
      vscode.window.showErrorMessage(`Failed to load sfdx-project.json: ${error}`);
      return;
    }
    ...
    const changeType = changeTypeSelection.startsWith('Feature') ? 'feature' : 'fix';
    ...
    const breakingChangeSelection = await vscode.window.showQuickPick(
      ['No breaking changes', 'Yes - includes breaking changes (major version)'],
      { 
    ...
    for (const pkg of modifiedPackages) {
      ...
      versionUpdates.push(`${pkg.packageName}: ${pkg.versionNumber} → ${newVersion}`);
    }
    ...
    await vscode.window.withProgress({
      ...
      for (const pkg of modifiedPackages) {
        ...
        if (packageIndex !== -1) {
          (projectContents.packageDirectories[packageIndex] as any).versionNumber = newVersion;
        ...
        if (settings?.enableReadmeGeneration !== false) {
          const readmePath = await changelogService.updatePackageReadme(pkg, options, settings);
