  ...
  try {
    ...
    json.packageDirectories.forEach((dir: any) => {
      if (dir.package) {
        ...
        let lineNumber = 0;
        const packagePattern = new RegExp(`"package"\\s*:\\s*"${dir.package.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"`);
        ...
        for (let i = 0; i < lines.length; i++) {
          if (packagePattern.test(lines[i])) {
            ...
            const afterPackageDirectories = beforeText.lastIndexOf('"packageDirectories"');
            ...
            if (afterDependencies > afterPackageDirectories) {
              const textSinceDependencies = beforeText.substring(afterDependencies);
              const openBrackets = (textSinceDependencies.match(/\[/g) || []).length;
              const closeBrackets = (textSinceDependencies.match(/\]/g) || []).length;
...
export function findPackageByLine(packages: PackageInfo[], lineNumber: number): PackageInfo | undefined {
