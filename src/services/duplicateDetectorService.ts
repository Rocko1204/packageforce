...
export class DuplicateDetectorService {
  ...
  private constructor() {
    this.outputChannel = vscode.window.createOutputChannel(
      'Packageforce Duplicate Detector'
    );
  }
  ...
  public async findDuplicates(
    ...
    logger.info(
      `Starting duplicate detection for package: ${options.packageName}`
    );
    this.outputChannel.clear();
    this.outputChannel.show(true); // Show output channel and preserve focus
    this.outputChannel.appendLine(
      `=== Duplicate Detection for ${options.packageName} ===`
    );
    this.outputChannel.appendLine(
    ...
    try {
      ...
      this.outputChannel.appendLine(
        'Scanning for Apex files (excluding test classes)...'
      );
      ...
      if (packageApexFiles.length === 0) {
        this.outputChannel.appendLine(
          'No non-test Apex files found in package.'
        );
      ...
      this.outputChannel.appendLine(
        `Found ${packageApexFiles.length} non-test Apex files in package:`
      );
      if (packageResult.excludedCount > 0) {
        this.outputChannel.appendLine(
          `  (Excluded ${packageResult.excludedCount} test files)`
        );
      }
      this.outputChannel.appendLine('');
      packageApexFiles.forEach(file => {
        this.outputChannel.appendLine(
          `  - ${path.relative(options.packagePath, file)}`
        );
      });
      ...
      this.outputChannel.appendLine(
        `Output length: ${cpdResult.length} characters`
      );
      if (cpdResult.length === 0) {
        this.outputChannel.appendLine(
          '⚠️  WARNING: CPD returned empty output!'
        );
        ...
        if (firstDuplication > 0) {
          this.outputChannel.appendLine(
            cpdResult.substring(0, Math.min(1000, firstDuplication + 500)) +
              '\n... (truncated) ...'
          );
        } else {
          this.outputChannel.appendLine(
            'No <duplication> tags found in CPD output'
          );
      ...
      const result = await this.parseCPDResult(
        cpdResult,
        packageApexFiles,
        options
      );
        ...
        if (stats.isDirectory() && !file.startsWith('.')) {
          ...
          if (file.endsWith('.cls')) {
            try {
              ...
              logger.warn(
                `Could not read file ${filePath} to check for @isTest: ${error}`
              );
            }
          }
   ...
   */
  private async getApexFilesFromWorkspace(
    workspaceRoot: string,
    excludePath: string
  ): Promise<string[]> {
    const apexFiles: string[] = [];
  ...
  private async runCPD(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.outputChannel.appendLine(
        `Minimum tokens: ${options.minimumTokens || 100}`
      );
  ...
  private async parseCPDResult(
    ...
    this.outputChannel.appendLine(
      `Total package files: ${packageFiles.length}`
    );
    if (packageFiles.length <= 10) {
      ...
      packageFiles
        .slice(0, 5)
        .forEach(f => this.outputChannel.appendLine(`  ${f}`));
      this.outputChannel.appendLine(
        `  ... and ${packageFiles.length - 5} more files`
      );
    }
    ...
    for (const match of duplicationMatches) {
      ...
      if (matchArray.length === 0) {
        fileMatches = content.matchAll(/<file[^>]+path="([^"]+)"[^>]*>/g);
      ...
      if (totalDuplicationsFound <= 3 && matchArray.length === 0) {
        this.outputChannel.appendLine(
          `  WARNING: No file matches found in duplication content`
        );
      }
      ...
      const isSimplePattern =
        matchArray.length > 0 && matchArray[0][2] === undefined;
      ...
      for (const fileMatch of matchArray) {
        ...
        if (isSimplePattern) {
          ...
          endLine = endLineMatch
            ? parseInt(endLineMatch[1])
            : startLine + lines - 1;
        } else {
      ...
      const codeMatch = content.match(
        /<codefragment>[\s\S]*?<!\[CDATA\[([\s\S]*?)\]\]>[\s\S]*?<\/codefragment>/
      );
      ...
      if (hasPackageFile) {
        ...
        const packageOccurrences = occurrences.filter(occ =>
          packageFiles.some(pkgFile => {
            ...
            return (
              normalizedPkgFile === normalizedFilePath ||
        ...
        if (totalDuplicationsFound <= 5) {
          if (packageOccurrences.length === occurrences.length) {
            this.outputChannel.appendLine(
              `  → Internal duplication (within package only)`
            );
          } else {
            this.outputChannel.appendLine(
              `  → Cross-package duplication (${packageOccurrences.length} in package, ${occurrences.length - packageOccurrences.length} in other packages)`
            );
          }
        }
      }
    ...
    this.outputChannel.appendLine(
      `Total duplications found in CPD output: ${totalDuplicationsFound}`
    );
    this.outputChannel.appendLine(
      `Duplications involving package files: ${duplications.length}`
    );

    // Check if PrepareEventInstanceHelper is in the CPD output at all
    if (xmlOutput.includes('PrepareEventInstanceHelper')) {
      this.outputChannel.appendLine(
        `✓ PrepareEventInstanceHelper found in CPD output`
      );

      // Count how many times it appears
      const count = (xmlOutput.match(/PrepareEventInstanceHelper/g) || [])
        .length;
      this.outputChannel.appendLine(`  Appears ${count} times in output`);
    } else {
      this.outputChannel.appendLine(
        `✗ PrepareEventInstanceHelper NOT found in CPD output`
      );
    }
  ...
  private displayResults(result: DuplicateResult) {
    ...
    if (result.totalDuplications === 0) {
      ...
      this.outputChannel.appendLine(
        'Your package code appears to be unique across the repository.'
      );
    } else {
      ...
      this.outputChannel.appendLine(
        `⚠️  Found ${result.totalDuplications} duplicate code blocks\n`
      );
      this.outputChannel.appendLine(`Summary:`);
      this.outputChannel.appendLine(
        `  - Total duplicate blocks: ${result.totalDuplications}`
      );
      if (crossPackageCount > 0) {
        this.outputChannel.appendLine(
          `  - ❌ Cross-package duplicates: ${crossPackageCount} (CRITICAL - code shared between packages!)`
        );
      }
      if (internalCount > 0) {
        this.outputChannel.appendLine(
          `  - ⚠️  Internal duplicates: ${internalCount} (within the same package)`
        );
      }
      this.outputChannel.appendLine(
        `  - Total duplicated lines: ${result.totalDuplicatedLines}`
      );
      this.outputChannel.appendLine(
        `  - Total duplicated tokens: ${result.totalDuplicatedTokens}\n`
      );
      ...
      result.duplications.forEach((dup, index) => {
        this.outputChannel.appendLine(`\n--- Duplication #${index + 1} ---`);
        this.outputChannel.appendLine(
          `Size: ${dup.lines} lines, ${dup.tokens} tokens`
        );
        this.outputChannel.appendLine(
          `Found in ${dup.occurrences.length} locations:\n`
        );

        dup.occurrences.forEach((occurrence, occIndex) => {
          // Check if this occurrence is in the package being analyzed
          const isInPackage =
            occurrence.file.includes(result.packageName) ||
            occurrence.file.includes(result.packageName.replace(/-/g, '_'));
          const marker = isInPackage ? '📦' : '🔗';

          this.outputChannel.appendLine(
            `  ${occIndex + 1}. ${marker} ${occurrence.file}`
          );
          this.outputChannel.appendLine(
            `     Lines ${occurrence.startLine}-${occurrence.endLine}`
          );
        });
    ...
    this.outputChannel.appendLine(
      `Finished at: ${result.timestamp.toLocaleString()}`
    );
  }
   ...
   */
  public createDuplicateDiagnostics(
    ...
    for (const duplication of result.duplications) {
      for (const occurrence of duplication.occurrences) {
        ...
        const prefix = hasCrossPackageDuplication
          ? '❌ Cross-package duplicate'
          : '⚠️ Duplicate';
  ...
  private getPackageFromPath(filePath: string): string {
    ...
    const forceAppIndex = pathParts.findIndex(
      part => part === 'force-app' || part.endsWith('-app') || part === 'src'
    );
  ...
  public async saveResults(
    ...
    const filePath = await ReportUtils.saveDuplicateResults(
      result,
      packagePath,
      reportOptions
    );
