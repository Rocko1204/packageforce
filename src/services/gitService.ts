...
export class GitService {
  ...
  async getModifiedPackages(
  ...
  ): Promise<PackageChangeInfo[]> {
    const stagedFiles = await this.getStagedFiles();
    const packageChanges = new Map<string, PackageChangeInfo>();

    logger.debug(
      `Checking ${packageDirectories.length} package directories for changes`
    );
    ...
    for (const pkg of packageDirectories) {
      ...
      for (const file of stagedFiles) {
        ...
        if (
          normalizedFile.startsWith(packagePath + path.sep) ||
      ...
      if (modifiedFiles.length > 0) {
        logger.info(
          `Package ${pkg.package} has ${modifiedFiles.length} modified files`
        );
