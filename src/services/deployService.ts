...
export class DeployService {
  ...
  async deploy(options: DeployOptions): Promise<void> {
    ...
    DeployLogger.log(
      '💪 Put packages in queue and start deployment/installation process'
    );
    ...
    try {
      ...
      const packageDependencyTree = getDeployUrls(
        this.project,
        options.packageName
      );
      ...
      if (!packageDependencyTree) {
        throw new Error(
          `Package "${options.packageName}" not found in sfdx-project.json`
        );
      }
      ...
      if (
        ...
        packageDependencyTree.dependency &&
        packageDependencyTree.dependency.length > 0
      ) {
        const depsList: string[] = packageDependencyTree.dependency.map(
          dep => dep.packagename || ''
        );
        ...
        for (const dep of packageDependencyTree.dependency) {
          ...
          if (options.startFrom && !isStarted) {
            if (options.startFrom !== dep.packagename) {
              ...
              DeployLogger.warning(
                `Restarting the deployment at the point of package ${options.startFrom}...👆`
              );
              isStarted = true;
            }
          }
          ...
          if (dep.packagename && dep.path) {
            await this.deployPackageTreeNode(
              dep.packagename,
              dep.path,
              options
            );
      ...
      // Deploy main package
      DeployLogger.notify(
        `Deploy package ${packageDependencyTree.packagename}...👇`
      );
      if (packageDependencyTree.packagename && packageDependencyTree.path) {
        await this.deployPackageTreeNode(
          packageDependencyTree.packagename,
          packageDependencyTree.path,
          options
        );
      }
      ...
      vscode.window.showInformationMessage(
        `Successfully deployed ${options.packageName}`
      );
    } catch (error) {
  ...
  private async setupOrg(targetOrg?: string): Promise<void> {
      ...
      try {
        ...
        if (!defaultOrgAlias) {
          ...
          if (orgs && orgs.length > 0) {
            ...
            DeployLogger.warning(
              `No default org set, using first available org: ${orgAlias}`
            );
          } else {
            throw new Error(
    ...
    try {
      ...
      if (
        ...
        error.message.includes('NamedOrgNotFoundError')
      ) {
        ...
        if (orgs && orgs.length > 0) {
          ...
          const matchingOrg = orgs.find(
            org => org.username === orgAlias || org.aliases?.includes(orgAlias)
          );

          if (matchingOrg) {
            this.org = await Org.create({
              aliasOrUsername: matchingOrg.username,
            });
            ...
            DeployLogger.warning(
              `Could not find org '${orgAlias}', using first available org: ${orgs[0].username}`
            );
          }
        } else {
          throw new Error(
  ...
  private async deployPackageTreeNode(
    pck: string,
    path: string,
    ...
    if (options.checkOnly) {
      DeployLogger.notify(
        `Validating package ${pck}... This is a check-only deployment.`
      );
    }
    ...
    deploy.onUpdate(response => {
      if (counter === 5) {
        ...
        DeployLogger.trace(
          `Deploy Package: ${pck} Status: ${status} Progress: ${progress}`
        );
        counter = 0;
      } else {
        counter++;
  ...
  private async displayErrors(
    ...
    DeployLogger.error(
      'Validation failed. Please check the deployment status in your org.'
    );
  }
}
