...
export class PoolService {
  ...
  async initialize(targetDevHub: string): Promise<void> {
    try {
      ...
      throw new Error(`Failed to connect to Dev Hub '${targetDevHub}': ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  
  async getAllDevHubs(): Promise<{ username: string; alias?: string; isDefault?: boolean }[]> {
    ...
    try {
      ...
      logger.info(`Found ${orgs.length} authenticated org(s), checking for Dev Hubs...`);

      if(!orgs.some(org => org.isDevHub)) {
        logger.error('No Dev Hubs found. Please ensure you have at least one Dev Hub org authenticated.');
        throw new Error('No Dev Hubs found. Please ensure you have at least one Dev Hub org authenticated.');
      }
      ...
      for (const orgInfo of orgs) {
        try {
          ...
          if (orgInfo.isDevHub) {
            ...
            logger.info(`Found Dev Hub: ${orgInfo.username}${alias ? ` (${alias})` : ''}`);
          } else {
            logger.debug(`${orgInfo.username} is not a Dev Hub`);
          }
        } catch (error) {
          logger.debug(`Failed to check ${orgInfo.username}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      ...
      throw new Error(`Failed to get Dev Hubs: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  
  async fetchScratchOrg(options: PoolFetchOptions): Promise<string> {
    if (!this.devHubOrg) {
      throw new Error('Dev Hub not initialized. Please provide a target Dev Hub.');
    }
    ...
    try {
      ...
      if (!result.records || result.records.length === 0) {
        logger.error(`No available scratch orgs found with tag '${options.tag}'`);
        throw new Error(`No available scratch orgs found with tag '${options.tag}'`);
      }
      ...
      if (!scratchOrg.SfdxAuthUrl__c || !isValidSfdxAuthUrl(scratchOrg.SfdxAuthUrl__c)) {
        logger.error(`Scratch org ${scratchOrg.SignupUsername} does not have a valid SfdxAuthUrl`);
        throw new Error(`Scratch org ${scratchOrg.SignupUsername} does not have a valid SfdxAuthUrl. Please ensure the org has a valid auth URL in the pool.`);
      }
      ...
      logger.info(`Successfully allocated scratch org: ${scratchOrg.SignupUsername}`);
  ...
  async listScratchOrgs(options: PoolListOptions): Promise<ScratchOrgInfo[]> {
    ...
    try {
      logger.debug(`Query: ${query}`);
      const result = await conn.query<ScratchOrgInfo>(query);
      logger.info(`Found ${result.records.length} scratch orgs with tag '${options.tag}'`);
  ...
  private async authenticateUsingSfdxAuthUrl(scratchOrg: ScratchOrgInfo, alias?: string): Promise<void> {
    try {
      ...
      const oauth2Options = AuthInfo.parseSfdxAuthUrl(scratchOrg.SfdxAuthUrl__c);
      ...
      await AuthInfo.identifyPossibleScratchOrgs(resultWithClientSecret, authInfo);
      
      logger.info(`Successfully authenticated to scratch org: ${scratchOrg.SignupUsername}`);
      if (alias) {
      ...
      throw new Error(`Failed to authenticate to scratch org: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  
  private async updateAllocationStatus(scratchOrgId: string, status: string): Promise<void> {
    try {
      const conn = this.devHubOrg!.getConnection();
      ...
      logger.info(`Updated allocation status to '${status}' for scratch org ${scratchOrgId}`);
    } catch (error) {
      logger.warn(`Failed to update allocation status: ${error instanceof Error ? error.message : 'Unknown error'}`);
