...
export class TestService {
  ...
  public async runTestsWithCoverage(options: TestOptions): Promise<TestResult> {
    ...
    this.outputChannel.appendLine(
      `🧪 Running Tests for Package: ${options.packageName}`
    );
    this.outputChannel.appendLine(
      `📍 Mode: ${options.runAsync !== false ? 'Asynchronous' : 'Synchronous'}`
    );
    this.outputChannel.appendLine(`${'='.repeat(60)}`);
    this.outputChannel.appendLine('');
    ...
    try {
      ...
      const testClasses = await this.findTestClassesInPackage(
        options.packagePath
      );
      ...
      if (testClasses.length === 0) {
        ...
        this.outputChannel.appendLine(
          '💡 Make sure your test classes have @isTest annotation'
        );
        return {
          success: false,
          testsRun: 0,
      ...
      this.outputChannel.appendLine(
        `📋 Found ${testClasses.length} test class(es):`
      );
      ...
      const testClassIds = await this.getTestClassIds(
        testClasses.map(tc => tc.name)
      );
  ...
  private async establishConnection(targetOrg?: string): Promise<void> {
    ...
    if (!username) {
      ...
      try {
        ...
        if (defaultOrgAlias && typeof defaultOrgAlias === 'string') {
          ...
          try {
            ...
            const authInfo = await AuthInfo.create({
              username: defaultOrgAlias,
            });
            username = authInfo.getUsername();
          } catch (error) {
            // If that fails, check if it's already a username
            logger.warn(
              `Could not resolve org alias '${defaultOrgAlias}': ${error}`
            );
            ...
            if (orgs && orgs.length > 0) {
              ...
              if (matchingOrg) {
                ...
                this.outputChannel.appendLine(
                  `⚠️  Could not find org '${defaultOrgAlias}', using first available org: ${username}`
                );
              }
            } else {
              throw new Error(
                'No authenticated orgs found. Please authenticate with an org first.'
              );
            }
          }
        } else {
          ...
          if (orgs && orgs.length > 0) {
            username = orgs[0].username;
            this.outputChannel.appendLine(
              `ℹ️  No default org set, using first available org: ${username}`
            );
          } else {
            throw new Error(
              'No authenticated orgs found. Please authenticate with an org first.'
        ...
        throw new Error(
          `Failed to resolve default org: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }
    ...
    if (!username) {
      throw new Error(
        'No target org specified and no default org could be determined'
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Failed to connect to org '${username}': ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }

  private async findTestClassesInPackage(
    packagePath: string
  ): Promise<Array<{ name: string; isTest: boolean }>> {
  ...
  private async runTestsAsync(
    ...
    return {
      ...
      testsRun:
        testRunResult.CompletedList.length + testRunResult.FailedList.length,
  ...
  private async pollTestExecution(jobId: string): Promise<ApexTestQueueResult> {
    ...
    while (attempts < maxAttempts) {
      ...
      if (
        result.QueuedList.length === 0 &&
        result.ProcessingList.length === 0
      ) {
        this.outputChannel.appendLine('');
  ...
  private async checkTestStatus(jobId: string): Promise<ApexTestQueueResult> {
    ...
    const response =
      await this.connection.tooling.query<ApexTestQueueItem>(query);
  ...
  private async getTestFailures(
    ...
    const query =
      `SELECT ApexClass.Name, Outcome, MethodName, Message, StackTrace ` +
      `FROM ApexTestResult ` +
      `WHERE Outcome = 'Fail' ` +
  ...
  private async getCodeCoverage(
    ...
    const response =
      await this.connection.tooling.query<ApexCodeCoverageAggregate>(query);
    ...
    const overallCoverage =
      totalLines > 0 ? Math.floor((totalCovered / totalLines) * 100) : 0;
  ...
  private displayTestResults(result: TestResult): void {
    ...
    if (result.coverage) {
      ...
      this.outputChannel.appendLine(
        `Overall Coverage: ${result.coverage.overall}% ${overallEmoji}`
      );

      if (result.coverage.overall < 75) {
        this.outputChannel.appendLine(
          '⚠️  Warning: Coverage is below 75% requirement'
        );
      }
